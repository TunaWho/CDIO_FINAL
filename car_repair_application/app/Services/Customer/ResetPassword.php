...
class ResetPassword extends BaseService
{
    ...
    {
        $user = $this->model::where('email', $email)->where('role_id', \App\Enums\UserRole::USER)->first();
    ...
    {
        $user = $this->model::where('email', $request->email)->where('role_id', \App\Enums\UserRole::USER)->first();
